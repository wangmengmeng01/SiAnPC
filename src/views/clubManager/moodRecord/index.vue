<template>
  <div class="app-container">
    ...
    >
      <el-button slot="publishUserNickName" slot-scope="{row}" size="mini" type="text" @click="$router.push({path:'/user-manager/user-detail', query:{ uid: row.uid}})">{{ row.publishUserNickName }}</el-button>
      <el-button slot="userSaId" slot-scope="{row}" size="mini" type="text" @click="$router.push({path:'/user-manager/user-detail', query:{ uid: row.uid}})">{{ row.userSaId }}</el-button>
      ...
      <template slot="action" slot-scope="{ row }">
        <mood-detail :row="{...row}">查看详情</mood-detail>
      </template>
    </TablePage>
  </div>
</template>
  ...
  data() {
    return {
      ...
      filterForm: [
        ...
        },
        {
          ...
          options: [
            ...
            { value: 'OPEN', label: '全员可见' },
            { value: 'BOSS', label: '老板可见' }
          ],
          labelStyle: {
            display: 'inline-block',
            width: '100px',
      ...
      tableCols: [
        ...
        { prop: 'clubBossNickName', label: '俱乐部老板昵称' },
        { prop: 'viewAuthName', label: '谁可以看' },
        { prop: 'createTime', label: '心情发布时间' },
