<template>
  <div class="add-tag">
    ...
    <!-- button部分 -->
    <el-button :type="btnType" :size="btnSize" @click="visible = true"><slot /></el-button>
    ...
    <el-dialog title="新增标签" :visible.sync="visible" :close-on-click-modal="false" width="500px" @close="reset">
      ...
      <el-form ref="form" :rules="rules" :model="form" label-position="right" label-width="100px">
        <el-form-item label="标签名" prop="content">
          <el-input v-model="form.content" placeholder="请输入标签名（10个字以内）" maxlength="10" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        content: [{ required: true, message: '请输入标签名' }],
