<template>
  <div class="app-container audio-sign">
    <TablePage
      show-search
      :search-cols="4"
    ...
    >
      <add-text slot="table-header" @success="updateTablePageListParams({})">新增提词板</add-text>
...
export default {
  ...
  methods: {
    // 上架下架标签
    putOff(id, status) {
      this.$confirm(status === 0 ? `确定要下架该提词板吗？` : `确认要上架该提词板吗？`, '提示', {
