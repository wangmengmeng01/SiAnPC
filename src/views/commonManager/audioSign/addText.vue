<template>
  <div class="add-text">
    ...
    <!-- dialog部分 -->
    <el-dialog title="新增提词板" :visible.sync="visible" :close-on-click-modal="false" width="500px" @close="reset">

      <el-form ref="form" :rules="rules" :model="form" label-position="right" label-width="100px">
        <el-form-item label="提词板内容" prop="content">
          <el-input v-model="form.content" placeholder="请输入提词板内容（50个字以内）" type="textarea" :autosize="{ minRows: 2, maxRows: 5 }" maxlength="50" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        content: [{ required: true, message: '请输入提词板内容' }],
