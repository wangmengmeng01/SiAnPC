<template>
  <div class="audit-front-cover">
    ...
    <el-button v-if="[1, 2].includes(row.status)" type="text" size="mini" disabled>已操作</el-button>
    <el-button v-if="row.status === 0" type="danger" plain size="mini" @click="audit(2)">拒绝</el-button>
    <el-button v-if="row.status === 0" type="success" plain size="mini" @click="audit(1)">通过</el-button>
...
export default {
  ...
  methods: {
    audit(action) {
      if (action === 1) {
        this.loading = true
        API[this.$route.path].audit({ showIds: [this.row.showId], action: 1 }).then((res) => {
    ...
    submit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.loading = true
          API[this.$route.path].audit({ showIds: [this.row.showId], action: 2, remark: this.form.remark }).then((res) => {
            this.$message.success('审核完成')
            this.$emit('success')
            this.loading = false
            this.visible = false
