  ...
  methods: {
    audit(logic) {
      if (logic === 1) {
        this.$confirm(`确认要通过吗？`, '提示', {
          ...
          try {
            this.loading = true
            await API[this.$route.path].audit({ id: this.row.id, logic: logic })
    ...
    submit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.loading = true
          API[this.$route.path].audit({ id: this.row.id, logic: 0, auditRemark: this.form.auditRemark }).then((res) => {
            this.$message.success('审核完成')
            this.$emit('success')
            this.loading = false
          }).catch((err) => {
