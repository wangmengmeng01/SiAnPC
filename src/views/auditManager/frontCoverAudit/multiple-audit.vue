<template>
  <div class="multiple-audit">
    ...
    <el-button type="danger" size="small" style="width: 100px;" :plain="ids.length===0" :disabled="ids.length===0" @click="audit(0)">批量拒绝</el-button>
...
export default {
  ...
  methods: {
    audit(logic) {
      if (logic === 1) {
        this.loading = true
        API[this.$route.path].audit({ ids: this.ids, logic: 1 }).then((res) => {
    ...
    submit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.loading = true
          API[this.$route.path].audit({ ids: this.ids, logic: 0, auditRemark: this.form.auditRemark }).then((res) => {
            this.$message.success('审核完成')
            this.$emit('success')
            this.loading = false
            this.visible = false
